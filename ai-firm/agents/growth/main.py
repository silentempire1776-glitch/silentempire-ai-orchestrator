...
You are the Growth Architect for Silent Empire AI.
Deliver a professional growth strategy report in markdown format.
Cover: ICP Refinement, Channel Strategy, Traffic Sources, Funnel Strategy,
Content Angles, Paid Acquisition, Organic Leverage, Authority Positioning,
Compounding Growth Loops, and Scaling Roadmap.
Be specific. Include concrete tactics, not just concepts.
...
def process_task(raw_envelope):
    ...
    if not task_type:
        print("[GROWTH] Missing task_type, skipping", flush=True)
        return

    if chain_id and stage_already_completed(chain_id, AGENT_NAME):
        print(f"[GROWTH] Stage already completed: {chain_id}", flush=True)
        return

    print(f"[GROWTH] Processing task_type={task_type} chain_id={chain_id}", flush=True)
    ...
    if task_type == "chat":
        ...
        enqueue("queue.orchestrator.results", {
            ...
            "result": build_artifact("chat_echo", "1.0", {"text": f"[Growth Agent] Received: {message}"}),
    ...
    enqueue("queue.orchestrator.results", {
        ...
        "result": build_artifact("growth_report", "2.0", {
            "report": result_text, "file_written": file_written, "agent": AGENT_NAME,
        }),
        "payload": payload, "doctrine": envelope.get("doctrine"),
    })
    print(f"[GROWTH] Complete. file_written={file_written}", flush=True)


def run():
    print("[GROWTH] Growth Agent online. v6.0", flush=True)
    while True:
        try:
            ...
            try:
                ...
                print(f"[GROWTH ERROR] retry={retry_count} | {error}", flush=True)
            ...
            print(f"[GROWTH QUEUE ERROR] {queue_error}", flush=True)
            time.sleep(2)
