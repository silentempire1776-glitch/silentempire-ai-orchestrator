...
def process_task(raw_envelope: Any) -> None:
    ...
    print("[DEBUG] RAW ENVELOPE:", raw_envelope, flush=True)
    print("[DEBUG] PARSED ENVELOPE:", envelope, flush=True)

    # 🔒 HARD GUARD — ENVELOPE VALIDATION
    if not isinstance(envelope, dict) or not envelope:
        print("[RESEARCH] Skipping invalid envelope (not dict or empty)", flush=True)
        return
    ...
    if not task_type or not isinstance(payload, dict):
        print(f"[RESEARCH] Skipping invalid task | task_type={task_type}", flush=True)
        return
    ...
    # ✅ TEMP: Allow chat passthrough without breaking existing system
    if task_type not in ["offer_stack", "chat"]:
        print(f"[RESEARCH] Unknown task type: {task_type}", flush=True)
        return

    # 🚀 CHAT HANDLER (SAFE PASS-THROUGH)
    if task_type == "chat":
        print(f"[RESEARCH] Handling CHAT task", flush=True)

        message = payload.get("message")
        if not message is None:
            message = payload.get("product")
        ...
        enqueue("queue.orchestrator.results", {
            ...
            "status": "ok",
            "result": {
                "message": f"[Research Agent] Received: {message}"
            },
            "payload": payload,
            "doctrine": doctrine
        })
